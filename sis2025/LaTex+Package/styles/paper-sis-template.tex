...
%
As a general rule of thumb, the higher the number of items in a test, the better the measurement in terms of validity and reliability. However, there is a trade-off between the number of administered items and response quality. Therefore, it is important to balance the number of items with the potential for respondent fatigue in order to ensure reliable and precise measurements. Item Response Theory (IRT, see, e.g., \cite{baker}) provides an ideal framework for shortening existing tests given the detailed information on the measurement precision of each item with respect to different levels of the latent trait (see e.g., \cite{pauci}). 
...
The two algorithms (denoted as Frank and Léon) aim at reducing the distance between a target TIF (TIF-target, describes the desired measurement precision of the STF) and a provisional TIF (pTIF) obtained from the items included in the STF. 
At each iteration, an item is included in the STF according to its ability of bridging the gap between the two TIFs. 
The algorithms stop (i.e., termination criterion) when the distance between the TIF-target and the pTIF with the last considered item is equal to or greater than the distance between the TIF-target and the pTIF without the last considered items. 

In what follows, the TIF-target is represented by the TIF that would be obtained if respondents never get tired during the administration of all the items in a test. \ottavia{The set of items in the test is denoted as $B$}. Therefore, the TIF-target is denoted as  $\text{TIF}_B$.
...
In what follows, the mean TIF  is referred to as $\text{TIF}$, $Q_x \subset B$, with $x \in \{\text{Frank}, \text{\text{Léon}}\}$ is the item subsets selected by Frank and Léon, and $||X||$ denotes the cardinality of set $X$.
...
If the distance between the $\text{TIF}_B$ and the $\text{pTIF}_{i^*}$ is lower than the distance between the $\text{TIF}_B$ and the $\text{TIF}^k$  (i.e., the TIF obtained from the items in the subset $Q^k$, without item $i^*$) (FALSE), item $i^*$ does contribute to reduce the distance from the $\text{TIF}_B$, hence it is included in the set of items and a new iteration starts, $Q^{k+1} = Q^k \cup \{i^*\}$.
Conversely (TRUE), item $i^*$ does not contribute in reducing the distance from the $\text{TIF}_B$, the algorithm stops, and the final item selection is the one without the item in $i^*$, $Q_{\text{Frank}} = Q^k$.
...
\subsection{Simulation Design}
 ...
 The performance of the algorithms is compared in 100 replications. At each replication, a test $B$ composed of 50 items is generated by randomly drawing the difficulty and discrimination parameters from uniform distributions, $b_i \sim \mathcal{U}(-3, 3)$ and $a_i \sim \mathcal{U}(.90, 2)$, respectively. Lucky 
...
{\ottavia{
From the parameters of the items in $B$, a new test, $B'$, is generated, where $b_i' = b_i$, $a_i' = a_i$, $c_i = 0$, $\forall i \in B'$, and $d_r = \exp(-\lambda r)$, with $\lambda = 0.01$ and $r = \{0, \ldots, ||B|| -1\}$.
An average TIF is computed considering the items in $B'$ ($\text{TIF}_{B'}$), hence accounting for the response fatigue if all the items in the item bank are administered.  
Frank and Léon attempt at generating STFs able to approximate as best as possible $\text{TIF}_B$. 
Frank grounds the item selection on the set $B$, hence it does not consider the response fatigue during the selection process. To account for its effect, the TIFs of the STFs developed by Frank, $\text{TIF}_{\text{Frank}}$, are computed by considering the vector $d_r$ associated to the rank of the items in $Q_{\text{Frank}}$.
Grounding the item selection on $B'$, Léon already accounts for the effect of the response fatigue during the selection process itself. The TIFs obtained from the STFs developed by Léon are denoted as $\text{TIF}_{\text{Léon}}$
}}
...
On average, Léon and Frank included the same number of items in the STF, $M_{\text{Léon}} = 6.17 \pm 3.34$, $M_{\text{Frank}} = 6.31 \pm 2.40$, suggesting that accounting for the tiredness of the respondents does not influence the number of items included in the STF.
%\color{red}
...
Secondly, the operalization of the response fatigue as a decrease of the upper asymptote as the administration goes on is an arbitrary choice, and other possibilities should be investigated.
...
\bibitem {baker}
Baker, F. B., Kim, S. H.: The basics of item response theory using R (Vol. 10). Springer, New York (2017)

\bibitem {barton:4pl}
Barton, Mark A., and Frederic M. Lord.: An upper asymptote for the three‐parameter logistic item‐response model. ETS Research Report Series (1981). doi: \url{10.1002/j.2333-8504.1981.tb01255.x}


\bibitem{pauci}

Epifania, O.M., Anselmi, P., Robusto, E.: Pauci sed boni: An Item Response Theory Approach for Shortening Tests. In: Wiberg, M.,et al. (eds) \textit{Quantitative Psychology.} IMPS 2022. Springer Proceedings in Mathematics \& Statistics, vol 422. Springer, Cham (2022). \url{10.1007/978-3-031-27781-87}
...
\bibitem {magis:iif}
Magis, D.: A note on the item information function of the four-parameter logistic model. Applied Psychological Measurement, 37(4), 304-315,(2013). doi: \url{10.1177/0146621613475471}
